...
public class Administrator extends JFrame {
	...
	public void PatientInfo() {
		...
		panel.add(FirstName);
		panel.add(lname);
		FirstName.setFont(LastName.getFont().deriveFont(15f));
		panel.add(LastName);
		panel.add(dob);
		...
		save.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String patientFName = FirstName.getText();
				...
				patient1.setFirst_name(patientFName);
				patient1.setLast_name(patientLName);
				patient1.setdate(date);
				patient1.setPhone_number(phone);
				patient1.setInsurance(carrier);
				patient1.setPatientType(ptype);
				patient.setFirst_name(patientFName);
				patient.setLast_name(patientLName);
				...
				if (ptype == "INPATIENT") {
					...
					System.out.println("Bed No." + bed + " is assigned to "
	...
	public TableModel InpatientTable() {
		...
		DefaultTableModel model = new DefaultTableModel(new Object[] { "Name",
				"Date Of Birth", "Phone Number", "Insurance", "Patient Type",
				"BED" }, 0);

		model.addRow(new Object[] {
				patient.getFirst_name().substring(0, 1).toUpperCase()
						+ patient.getFirst_name().substring(1),
				patient.getLast_name().substring(0, 1).toUpperCase()
						+ patient.getLast_name().substring(1),
				patient.getdate(),
				patient.getPhone_number(),
				patient.getInsurance().substring(0, 1).toUpperCase()
						+ patient.getInsurance().substring(1),
				patient.getPatientType(), patient.getAssignedBed() });
	...
	public void optionChecker() {
		...
		do {
			System.out
					.println("-----------------------choose any one of the following");
